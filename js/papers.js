...
const papersData = [
    {
        ...
        doi: null,
        pdf: "https://ieeexplore.ieee.org/document/11146867",
        code: "https://github.com/RogsXie/DAHGMN",
        keywords: ["Attention mechanism, graph convolution network, joint classification, light detection and ranging, Mamba"],
        // Model image fields
        hasModelImage: true,
    ...
    },
    {
        ...
        publicationGroup: "firstSecondAuthor",
        doi: null,
        pdf: "https://ieeexplore.ieee.org/document/11397521",
        code: "https://github.com/RogsXie/CPCM",
        keywords: ["Contrastive learning, joint clustering, mamba, multimodal remote sensing, prototypical learning, spectral-spatial"],
        // Model image fields
        hasModelImage: true,
...
// ===== Helper functions for Zhenyang Xie author position =====
function getZhenyangXieAuthorPosition(authors) {
    return authors.split(',').map(author => author.trim()).findIndex(author => author === 'Zhenyang Xie');
}

function isZhenyangXieFirstOrSecondAuthor(authors) {
    const position = getZhenyangXieAuthorPosition(authors);
...
function sortPapers(papers) {
    return papers.sort((a, b) => {
        const aAuthorPosition = getZhenyangXieAuthorPosition(a.authors);
        const bAuthorPosition = getZhenyangXieAuthorPosition(b.authors);
        const aIsPrimaryGroup = isZhenyangXieFirstOrSecondAuthor(a.authors);
        const bIsPrimaryGroup = isZhenyangXieFirstOrSecondAuthor(b.authors);
        
        // Priority 1: Zhenyang Xie's first-/second-author papers come first
        if (aIsPrimaryGroup && !bIsPrimaryGroup) return -1;
        if (!aIsPrimaryGroup && bIsPrimaryGroup) return 1;

        // Priority 2: earlier author position comes first
        if (aAuthorPosition !== bAuthorPosition) {
            if (aAuthorPosition === -1) return 1;
            if (bAuthorPosition === -1) return -1;
            return aAuthorPosition - bAuthorPosition;
        }
        
        // Priority 3: newest work first, then citations and impact factor
...
function renderPapers(papers) {
    ...
    const primaryPapers = papers.filter(paper => paper.publicationGroup !== 'coauthored');

    container.innerHTML = `
        ${createPublicationGroupHeader('firstSecondAuthor')}
        ${primaryPapers.map(paper => createPaperCard(paper)).join('')}
        ${createPublicationGroupHeader('coauthored')}
    `;
}

// ===== Helper function to highlight specific author =====
function highlightAuthor(authors) {
    // Highlight "Zhenyang Xie" with bold font
    return authors.replace(/Zhenyang Xie/g, '<strong style="font-weight: 700; color: #0056b3;">Zhenyang Xie</strong>');
}

// ===== Create publication group heading =====
function createPublicationGroupHeader(groupKey) {
    ...
    const label = groupKey === 'coauthored' ? t.publications.coauthored : t.publications.firstSecondAuthor;
    const icon = groupKey === 'coauthored' ? 'fa-users' : 'fa-user-pen';
